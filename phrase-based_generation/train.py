...
if __name__ == '__main__':
    ...
    assert args.mode in ["ph_bc", "bc", "ph", "ph_bc_only_pop909"]

    if args.wandb:
        wandb.login()
        wandb.init(project="music_generator_train")

    train_dataset = music_data.MusicDataset(
        pkl_path=f"{args.mode}/data_pkl/train_{args.max_len}.pkl",)

    test_dataset = music_data.MusicDataset(
        pkl_path=f"{args.mode}/data_pkl/test_{args.max_len}.pkl",)
    ...
    for epoch in tqdm(range(start_epoch, args.epoch_num)):
        ...
        if epoch % 1 == 0:
            ...
            torch.save(checkpoint, f'{args.mode}/ckpt/generator_{epoch}.pt')
