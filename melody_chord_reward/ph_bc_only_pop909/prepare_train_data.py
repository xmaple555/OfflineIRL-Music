...
def prepare_melody_train_data():
    ...
    for_loop_items = [[ph_bc_melody_train_dataset, train_full_seqs],
                      [ph_bc_melody_test_dataset, test_full_seqs],
                      [ph_bc_melody_sampled_dataset, sampled_seqs]]

    for pc_ph_dataset, full_seqs in for_loop_items:

        for data in tqdm(full_seqs, desc="Processing Melody Data"):
            if data['name'] not in ["pop909_1", "pop909_2"]:
                continue
            seq = data["seq"]
            section_start_idics = []
            section_end_idics = []
    ...
    with open(f"data_pkl/melody_train.pkl", "wb") as f:
        pickle.dump(ph_bc_melody_train_dataset, f)
    with open(f"data_pkl/melody_test.pkl", "wb") as f:
        pickle.dump(ph_bc_melody_test_dataset, f)
    with open(f"data_pkl/melody_sampled.pkl", "wb") as f:
...
def prepare_chord_train_data():
    ...
    for_loop_items = [[ph_bc_chord_train_dataset, train_full_seqs],
                      [ph_bc_chord_test_dataset, test_full_seqs],
                      [ph_bc_chord_sampled_dataset, sampled_seqs]]

    for ph_bc_dataset, full_seqs in for_loop_items:
    ...
    with open(f"data_pkl/chord_train.pkl", "wb") as f:
        pickle.dump(ph_bc_chord_train_dataset, f)
    with open(f"data_pkl/chord_test.pkl", "wb") as f:
        pickle.dump(ph_bc_chord_test_dataset, f)
    with open(f"data_pkl/chord_sampled.pkl", "wb") as f:
