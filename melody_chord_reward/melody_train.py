...
if __name__ == '__main__':
    ...
    if args.wandb:
        wandb.login()
        wandb.init(project="melody_model_train")

    train_dataset = music_data.MelodyDataset(
        pkl_path=f"{args.mode}/data_pkl/melody_train.pkl",)

    test_dataset = music_data.MelodyDataset(
        pkl_path=f"{args.mode}/data_pkl/melody_test.pkl",)

    sampled_dataset = music_data.MelodyDataset(
        pkl_path=f"{args.mode}/data_pkl/melody_sampled.pkl",)
    ...
    model = MelodyModel(myvocab.n_tokens).cuda()
    model.train()
    ...
    for epoch in tqdm(range(start_epoch, args.epoch_num)):
        ...
        if epoch % 1 == 0:
            ...
            torch.save(checkpoint,
                       f"{args.mode}/ckpt/melody_model/melody_model_{epoch}.pt")
