    ...
    def __init__(self,
        ...
        if custom_decoder is not None:
            ...
            decoder_layer_1 = TransformerDecoderLayer_parallel(
                d_model, nhead, dim_feedforward, dropout, activation)
            decoder_layer_2 = TransformerDecoderLayer(d_model, nhead,
                                                      dim_feedforward, dropout,
                                                      activation)
            ...
            self.decoder = TransformerDecoder_Hybrid(
                ...
                decoder_patten, xor_pattern, decoder_norm)
...
class TransformerDecoderLayer(Module):
    ...
    def forward(self,
        ...
        tgt = self.norm2(tgt)
        if hasattr(self, "activation"):
            tgt2 = self.linear2(self.dropout(self.activation(
                self.linear1(tgt))))
        else:  # for backward compatibility
...
class TransformerDecoderLayer_parallel(Module):
    ...
    def forward(self,
        ...
        tgt2_self = self.self_attn(tgt2_self,
                                   tgt2_self,
                                   tgt2_self,
                                   attn_mask=tgt_mask,
                                   key_padding_mask=tgt_key_padding_mask)[0]
        ...
        gate_msk = ((tgt_label > 0).unsqueeze(2).expand(-1, -1,
                                                        tgt2_cross.shape[2]))
        assert (gate_msk.dtype == torch.bool)

        gate_msk = (tgt_label.unsqueeze(2).expand(-1, -1,
                                                  tgt2_cross.shape[2])).bool()
